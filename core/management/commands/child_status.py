...
class Command(BaseCommand):
    help = (
        "הצג מצב נוכחי של התינוק - מה קרה לאחרונה ומה צפוי / Show current baby status"
    )
    ...
    def handle(self, *args, **options):
        ...
        if next_feeding:
            ...
            if next_feeding["status"] == "overdue":
                self.stdout.write(self.style.ERROR(f'   ⚠️  {next_feeding["message"]}'))
        ...
        if last_sleep:
            ...
            self.stdout.write(
                f'   ⏱️  התעורר לפני {last_sleep["time_since_formatted"]} שעות'
            )
            self.stdout.write(f'   ⌛ משך: {last_sleep["duration_minutes"]:.0f} דקות')
            self.stdout.write(
        ...
        if next_sleep:
            ...
            if next_sleep["status"] == "overtired":
                self.stdout.write(self.style.ERROR(f'   ⚠️  {next_sleep["message"]}'))
            elif next_sleep["status"] in ["getting_tired", "soon"]:
                self.stdout.write(self.style.WARNING(f'   😴 {next_sleep["message"]}'))
            else:
        ...
        self.stdout.write("\n" + "=" * 70)
        self.stdout.write(
            self.style.SUCCESS("✅ סיכום מצב הושלם / Status summary completed")
        )
        self.stdout.write("=" * 70 + "\n")
