...
class Command(BaseCommand):
    help = (
        "הצג סיכום יומי של פעילויות התינוק / Display daily summary of baby activities"
    )
    ...
    def handle(self, *args, **options):
        ...
        if days == 1:
            ...
            self.stdout.write(f"\n🍼 האכלות:")
            self.stdout.write(
                f'   • ממוצע בין האכלות: {feeding_stats["average_interval_minutes"]:.1f} דקות'
            )
            self.stdout.write(
                f'   • ממוצע משך האכלה: {feeding_stats["average_duration_minutes"]:.1f} דקות'
            )
            ...
            self.stdout.write(f"\n💤 שינה:")
            self.stdout.write(
                f'   • ממוצע שעות שינה ביום: {sleep_stats["average_sleep_hours_per_day"]:.1f} שעות'
            )
            self.stdout.write(
                f'   • ממוצע משך תנומה: {sleep_stats["average_nap_duration_minutes"]:.1f} דקות'
            )
